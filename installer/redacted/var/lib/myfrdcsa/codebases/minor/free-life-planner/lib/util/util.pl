...
wot(Item,A) :-
	with_output_to(atom(A),call(Item)).

view(Item) :-
	write_term(Item,[quoted(true)]),nl,!.

view_pretty(Item) :-
	print_term(Item,[write_options([quoted(true)])]),nl,!.
...
allTermAssertions(Term,Assertions) :-
	allTermAssertionsNoDynamic(Term,Assertions).

allTermAssertionsNoDynamic(Term,Assertions) :-
	listFiles(Files),
	setof(C,File^Files^M^P^B^R^(member(File,Files),atom(File),view([file,File]),predicate_property(M:P,file(File)),clause(P,B,R),C = :-(P,B)),Cs),
...
%% 2020-10-25 16:57:25 <aindilis> is there a way to quickly get the list of all
%%       facts mentioning a given term?
%% 2020-10-25 16:57:48 <aindilis> I was hoping in an indexed fashion so the
%%       results would be almost instantaneous
%% 2020-10-25 16:58:30 <anniepoo_> current_predicate will give you predicate
%%       signatures, then clause will give you heads and bodies
%% 2020-10-25 16:58:46 <aindilis> yeah that's what I'm doing now, I was hoping
%%       for a built-in
%% 2020-10-25 16:58:57 <anniepoo_> an alternative if they're not dynamic is to
%%       use term_expansion
%% 2020-10-25 16:59:37 <anniepoo_> don't actually make a change (indeed you can
%%       fail) but add the clause to some db
%% 2020-10-25 17:01:53 <aindilis> awesome, ty for the help, noting this in the
%%       file
...
decamelcaseIntoList(Input,List) :-
	...
	viewIf([input,Input,word,Word,new,New]),
...
getNextWord(Atom,Word,NewAtom) :-
	...
	regex_atom("^([A-Za-z0-9][a-z]*)",[],Atom,[Word]),
	atom_length(Word,WordLength),
	atom_length(Atom,AtomLength),
	viewIf([atom,Atom,word,Word,atomLength,AtomLength,wordLength,WordLength]),
...
chomp_trailing_slash_from_path(Path,NewPath) :-
	(   (	regex_atom('^(.+)/+$',[s],Path,[Output]),
		atom_string(TmpNewPath,Output)) ->
	    NewPath = TmpNewPath ;
	    NewPath = Path).

list_prefix(N,List,Prefix) :-
...
ensureAsserted(Fact) :-
	(   not(Fact) -> (assert(Fact),write_term([fact1,Fact],[quoted(true)]),nl) ; true). %% view([notAsserted,Fact])).

ensureFAsserted(AgentName,FormalogName,Fact,Result) :-
	(   not(Fact) -> (fassert(AgentName,FormalogName,Fact,Result),write_term([fact1,Fact],[quoted(true)]),nl) ; true). %% view([notAsserted,Fact])).
...
prologify(Input,PrologifiedOutput) :-
	atom_string(Input,String),
	split_string(String, " ,_-&%", "", [HeadString|RestStrings]),
	atom_string(Head,HeadString),
	downcase_atom(Head,DHead),
	findall(CItem,(member(ItemString,RestStrings),atom_string(Item,ItemString),capitalize(Item,CItem)),CItems),
	append([[DHead],CItems],List),
	atomic_list_concat(List,'',PrologifiedOutput).

prologifyRest(Input,PrologifiedRestOutput) :-
	atom_string(Input,String),
	split_string(String, " ,_-&%", "", RestStrings),
	findall(CItem,(member(ItemString,RestStrings),atom_string(Item,ItemString),capitalize(Item,CItem)),CItems),
	atomic_list_concat(CItems,'',PrologifiedRestOutput).
...
list_adder([ ],0.0).
list_adder([X|L],Sum) :- list_adder(L,SL), Sum is float(SL + X).

list_subtracter([ ],0.0).
list_subtracter([X|L],Sum) :- list_subtracter(L,SL), Sum is float(SL - X).
...
%% 	clause(HH,BB),
%% 	H+B=@=HH+BB,
%% 	view([h,H,b,B,hh,HH,bb,BB]),
%% 	retract(HH).

to_head_body(H:-B,H,B) :- !.
to_head_body(H,H,true).
