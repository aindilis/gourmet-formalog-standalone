...
:- dynamic pattern_cache/3.		% Pattern, Flags, Regex

regex_replace(Input, Pattern, Replace0, Flags, Result) :-
	dollar_replace(Replace0, Replace),
	with_mutex(sparql_regex, locked_replace(Input, Pattern, Replace, Flags, Result)).
...
locked_replace(Input, Pattern, Replace, Flags, Result) :-
	...
	send(Regex, for_all, S, message(@(arg1), replace, @(arg2), Replace)),
	get(S, value, Result).
