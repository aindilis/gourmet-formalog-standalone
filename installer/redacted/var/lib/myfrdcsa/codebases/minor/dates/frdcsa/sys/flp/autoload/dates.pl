...
getCurrentDateTimeStamp(DateTimeStamp) :-
	getCurrentDateTime([Y-M-D,H:Mi:TmpS]),
	atom_number(Y1,Y),
	numberToAtomWithPrecedingZeroIfNecessary(M,M1),
	numberToAtomWithPrecedingZeroIfNecessary(D,D1),
	numberToAtomWithPrecedingZeroIfNecessary(H,H1),
	numberToAtomWithPrecedingZeroIfNecessary(Mi,Mi1),
	S is round(TmpS),
	numberToAtomWithPrecedingZeroIfNecessary(S,S1),
	atomic_list_concat([Y1,M1,D1,H1,Mi1,S1],'',DateTimeStamp).
...
%% currentTimeZone(easternStandardTime).
