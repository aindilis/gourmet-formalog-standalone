...
(global-set-key "\C-cgm." 'gourmet-formalog-act-on-thing-at-point)
...
(defun gourmet-formalog-select-windows ()
 ...
 (other-window 1)
 (ffap "/var/lib/myfrdcsa/codebases/minor/gourmet-formalog/scripts/gourmet/gourmet_logic.pl")
 ;; (ffap "/var/lib/myfrdcsa/codebases/minor/gourmet-formalog/scripts/gourmet/gourmet.pl")
...
(defun gourmet-formalog-act-on-thing-at-point ()
 ""
 (interactive)
 (let* ((fdc-id (substring-no-properties (thing-at-point 'symbol)))
	(results
	 (formalog-query
	  (list 'var-Results)
	  (list "get_desc_nutrition_and_ingredients_for_fdc_id" fdc-id 'var-Results)
	  nil "Gourmet-Formalog-Agent1"))
	(cleaned-results (-tree-map #'gourmet-formalog-clean-results results)))
  (see cleaned-results)))

(defun gourmet-formalog-clean-results (string)
 ""
 (replace-in-string string "\%" "_"))
